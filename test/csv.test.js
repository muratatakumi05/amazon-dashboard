...
test("日本語 Business Report の通貨とカンマを合計する", () => {
  ...
  assert.deepEqual(aggregateSales(csv), { total: 12350, rows: 2, header: "注文商品売上" });
});
...
test("日本語 Sponsored Products の標準ヘッダーを読み込む", () => {
  ...
  assert.deepEqual(aggregateAds(csv), { spend: 1500, sales: 5700, rows: 2, spendHeader: "費用", salesHeader: "7日間の合計売上" });
});
...
test("日本語広告レポートの「売上高」「総費用」という表記も読み込む", () => {
  ...
  assert.deepEqual(aggregateAds(csv), { spend: 2000, sales: 8000, rows: 1, spendHeader: "総費用", salesHeader: "7日間の合計売上高" });
});

test("英語ヘッダー、引用符内改行、エスケープ引用符を解析する", () => {
  const parsed = parseCsv('Campaign,Spend,Sales\n"A\n""special""",10,20');
  assert.equal(parsed.rows[0].Campaign, 'A\n"special"');
  assert.deepEqual(aggregateAds('Campaign,Spend,Sales\nA,10,20'), { spend: 10, sales: 20, rows: 1, spendHeader: "Spend", salesHeader: "Sales" });
});
